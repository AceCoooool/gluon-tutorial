...
class RNN(gluon.Block):
    ...
    def forward(self, x):
        # Set initial states
        h0 = nd.zeros((self.num_layers, x.shape[0], self.hidden_size), ctx=ctx)
        c0 = nd.zeros((self.num_layers, x.shape[0], self.hidden_size), ctx=ctx)
        # Forward propagate RNN
        out, _ = self.lstm(x, [h0, c0])
        return self.fc(out[:, out.shape[1] - 1, :])


rnn = RNN(hidden_size, num_layers, num_classes)
